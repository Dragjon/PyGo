...
def negamax(board, depth, color, alpha, beta, ply):
    if board.can_claim_draw() or board.is_stalemate() or board.is_insufficient_material():
            return 0
    ...
    if depth == 0 or board.is_game_over():
        return quiescence_search(board, color, alpha, beta, ply+1)
    ...
    for move in ordered_moves:
        board.push(move)
        value = -negamax(board, depth - 1, -color, -beta, -alpha, ply+1)
        board.pop()
...
def best_move(board, depth):
    ...
    # If no move is found in the opening book, use your existing logic
    ...
    for move in ordered_moves:
        ...
        if board.is_checkmate():
            return move
        eval = -negamax(board, depth - 1, -color, -beta, -alpha, ply+1)
        board.pop()
...
def play_chess():
    ...
    while not board.is_game_over():
        ...
        if board.turn == chess.BLACK:
            ...
            move_str = input("Enter your move (in UCI format): ")
